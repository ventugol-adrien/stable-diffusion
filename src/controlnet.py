...
class ControlNetAssetGenerator:
    ...
    def __init__(
        ...
        try:
            # We remove the hardcoded filename. controlnet_aux knows exactly
            # which file to download (table5_pidinet.pth) natively!
    ...
    def process(self, input_bytes: bytes) -> bytes:
        ...
        # Generate Crisp Edge map
        logger.info("Generating Crisp Edge map...")
        try:
            if self.edge_pipe:
                # controlnet_aux annotators directly take and return PIL Images
                edge_image = self.edge_pipe(
                    image,
                    detect_resolution=1024,  # SDXL/SOTA native resolution
                    image_resolution=1024,
                    safe=False,  # Set to True if you want slightly thicker, safer lines
                )
            else:
